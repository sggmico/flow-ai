...
- Token 优先: 允许多次查询, 默认最多 5 次
- 输出规范: 遵循 code-docs-creator
- 仓库名: 文章与 summary 中的仓库名默认小写优先, 必须严格遵守
- 语言: 生成的文档必须严格遵从中文优先的输出规则
...
### Step 2: 数据获取 (多次查询, 默认最多 5 次)

先扫描历史 summary.md, 排除已出现仓库, 再做查询。
...
**gh CLI 失败时的备选数据源 (优先级)**:

1. https://github.com/search (官方搜索功能)

**允许**: 多次查询、WebFetch、额外 API 调用 (默认最多 5 次)
...
> 数据截止: {日期} | 时间窗口: {window} | 生成模型: {model}
